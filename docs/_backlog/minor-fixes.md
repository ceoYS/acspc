...
**잔여 (MF-06 미완)**:
- ~~Photo.storage_path 형식 검증 (regex {user_id}/{project_id}/{id}.jpg)~~ — **완료 (D-4m, HEAD=99a9a74)**: zod regex 적용 + photo.test.ts 6 case (uppercase + invalid format 3종 커버)
...
- Vendor.name 특수문자 sanitize (`/\:*?"<>|`) — D-4j Codex H3, **재검토 필요 (D-4m 보류)**: 근거 약함 (storage_path 맥락 ≠ vendor 인적 표기). Vendor.name 은 파일 시스템 미진입 + 한글/특수기호 (`&`, `.`) 정상 입력 차단 위험. V1 후반에 정책 재정의 후 처리. 현재는 min(1).max(100) + blank-after-trim refine 으로 기본 안전망 유지.
- Photo.unique(project_id, name) — DB 레벨, V1 후반 (Phase 5 Supabase 도입 후)

**우선순위**: ~~Phase 5 진입 전 Photo.storage_path regex~~ → **D-4m 처리 완료**. 나머지 (H3 재검토, H4 unique) 는 V1 후반.
...
- 상태: completed (D-4l Chunk 1 종결, HEAD=694135a, 2026-04-28)
...
### 진행 상황 (D-4l Chunk 1, 2026-04-28)
- corp 망 정찰 (시나리오 A 확정): runs-on: ubuntu-latest 동작 OK, registry 접근 정상 (CA 주입 불필요), Secrets 권한 보유
- actions 메이저 핀 = v4 트리오 (pnpm.io 공식 docs 권장 보수적 선택). v6 마이그는 MF-28 후순위
- workflow yaml: actions/checkout@v4 + pnpm/action-setup@v4 (version: '10.33.0', packageManager 일치) + actions/setup-node@v4 (node-version-file: '.nvmrc', cache: 'pnpm')
- Codex CLI 외부 Evaluator 통과: Q1~Q5 ✅, Q6 조건부 risk 3건 (lockfile 정합성, pnpm PATH 순서, Node 22.22.2 배포 존재) 모두 mitigation 명확
- frozen-lockfile dry-run 통과 (DRY_RUN_OK, push 전 사전 확증)
- 1차 push STOP: PAT workflow scope 부재 (classic PAT 한계) → 본 D-4l 의 핵심 학습
- 복구: fine-grained PAT 신규 발급 (90일 만료, repo-specific = ceoYS/acspc only, Contents+Metadata+Workflows = Read/Write 최소 권한) → push 성공
- credential.helper = store 영구 저장 → 다음 push 부터 자동 인증 (90일 후 재입력 1회)
- CI 첫 run 결과: ✅ 성공, 50초 (추정 4~9분 대비 약 6배 빠름. GitHub 호스팅 runner = corp 망 무관)
- 종결: HEAD=694135a, MF-21 → completed, Phase 4.5 = 3/3 완료
...
### 진행 상황 (D-4k Chunk 3a, 2026-04-27)

상태: **완료**.

**도입 결정**:
- 도구: `simple-git-hooks@^2.13.1` 단일 (lint-staged 제거, prettier conflict + monorepo 분기 복잡도 회피)
- pre-commit 명령: `pnpm run check-types`
- 자동 등록: `scripts.prepare = "simple-git-hooks"` (npm/pnpm install lifecycle)
- 출처 docs 인용: github.com/toplenboren/simple-git-hooks README "Add simple-git-hooks to your package.json"

**검증 (Gate 2)**:
- `pnpm install` 14.3s 완료, prepare hook 정상 실행
- `.git/hooks/pre-commit` 생성 (230 bytes, 마지막 줄 `pnpm run check-types`)
- 정상 commit 시 `turbo run check-types` 3 successful → commit 통과
- HEAD: a3de823

**out of scope (Phase 4.5 제외, 별도)**:
- lint-staged + prettier 자동화 — MF-27 후보 (prettier config 정비 필요, V1 후반)
- 의도적 type-error reject 검증 — 실운영 흐름에서 자연 검증 (skip 합의)

**escape hatch** (긴급 시):
- 단발: `git commit --no-verify`
- 세션 단위: `export SKIP_SIMPLE_GIT_HOOKS=1`
...
## MF-28 (신규 — D-4l): GitHub Actions 메이저 v4 → v6 마이그
- 상태: pending (V1 후반 또는 2026-06 Node24 default 전환 시점)
- 우선순위: low (현 v4 트리오 정상 작동, 본 마이그는 미래 호환성)
- 발견 맥락: D-4l Chunk 1 actions 버전 결정 시 v4 (pnpm.io 공식 docs) vs v6 (marketplace latest) 선택. 사용자 원칙 "안정 우선" 으로 v4 채택. v6 마이그는 별도 chunk 로 분리
- 구체:
  * actions/checkout@v4 → @v6 (release v6.0.2 = 2026-01-09)
  * pnpm/action-setup@v4 → @v6 (Node24 호환)
  * actions/setup-node@v4 → @v6 (Node24 default)
- 진입 트리거:
  * 2026-06-02 Node24 default 전환 시 v4 deprecation 경고 발생 시
  * 또는 V1 후반 의존성 일괄 정비 시점
- 위험: v4 → v6 changelog 검토 필요. breaking change 가능성 (특히 setup-node 의 cache 동작 변경)

## MF-29 (신규 — D-4l): branch protection rules + status check 의무화
- 상태: pending (협업/V2 진입 시)
- 우선순위: medium (단독 작업 동안은 main 직접 push 가능)
- 발견 맥락: D-4l Chunk 1 CI 도입으로 push 마다 자동 검증되지만, CI 실패 commit 도 main 에 들어갈 수 있음 (CI 결과는 사후 통보). 협업 진입 시 main 보호 필요
- 구체:
  * Settings → Branches → Add rule for main
  * "Require status checks to pass before merging" 활성 + "smoke" job 필수 체크
  * "Require pull request before merging" 활성 (단독 작업 시 비활성 OK)
  * "Require linear history" 활성 (rebase / squash 만 허용)
- 진입 트리거:
  * V2 진입 또는 collaborator 추가 시
- 위험: 단독 작업 중 활성화 시 hotfix push 불가 → admin 우회 필요

## MF-30 (신규 — D-4l): dependabot.yml 도입
- 상태: pending (MF-29 후)
- 우선순위: low (의존성 자동 업데이트 PR)
- 발견 맥락: GitHub Actions marketplace actions 버전 핀 + npm 의존성 업데이트를 자동 PR 로 처리. MF-29 (PR 패턴 정착) 후 진입
- 구체:
  * .github/dependabot.yml 신규 생성
  * package-ecosystem: github-actions (월 1회) + npm (월 1회)
  * target: main
  * reviewers: ceoYS
- 진입 트리거: MF-29 종결 후 + V1 안정화 후
- 위험: 자동 PR 폭주 가능 (이미 알려진 문제) → schedule.interval = monthly 로 제한
