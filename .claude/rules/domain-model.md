...
## 6. 삭제 정책 [photo 단독 삭제: V1 보류 → V2]

V1: photo 단독 삭제 미지원. D-4u 결정 §b 로 V2 검토 후 재고
(회사망 Deno runtime 미확인, phase-5.5-decomposition.md v2 = 0cf629c).

- V1: **전체 hard delete**. soft delete 컬럼 없음.
- 프로젝트 삭제 시 cascade 로 하위 전부 삭제.
- Photo 단독 삭제 (V2 검토 후보): Supabase Storage 파일도 동시 삭제
  필요. 구현 후보 = **Edge Function** (DB trigger 는 Storage 접근
  복잡). 삭제 요청을 Edge Function 이 받아 DB row + Storage 파일
  트랜잭션. V1 에선 미구현, 클라이언트 직접 삭제도 금지.
