...
- 엑셀 양식 다양화 (docs/01 §4 V2+ 범위) 진입 시 exceljs 서버 측
  ...
  번들 크기 재계산.
- V1.5+ 모바일 진입 시 exceljs 실행 위치 (Next.js Node → Edge Function
  Deno) 재검토. 검토 트리거: 모바일 V1.5 chunk 진입 시점.
...
- 엑셀 생성은 Next.js 서버 측 (§1.6) — V1.5+ 모바일 진입 시 재검토
- 파일명 sanitization
- 사진 embed 전 전처리

### 1.6 엑셀 생성
- **exceljs** 4.4.0
- **실행 위치**: Next.js 서버 측 (API Route 또는 server action, Node 런타임)
- 모바일/웹 클라이언트는 엑셀 생성 트리거 + 결과 다운로드만 담당
- 사유: V1 = web 단독 사용 (모바일은 V1.5+ 진입). 모바일 진입 시 번들
  크기 절감 + Edge Function 분리 재검토 — §0 잠재 위험에 명시.
  양식 수정 시 앱 재배포 불필요는 V1 web 단독 구조에서도 유지됨
  (서버 코드만 재배포)
